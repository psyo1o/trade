...
국장 섹터
    * ``api.kr_stock_meta`` — KIS 업종명 → 네이버 ``industryCode``/ETF (yfinance 미사용, 404 방지).
"""
...
from api.kr_stock_meta import resolve_kr_sector as _resolve_kr_sector_meta
...
def get_kr_sector(symbol: str) -> str:
    ...
    sector = _resolve_kr_sector_meta(code, broker=broker)
