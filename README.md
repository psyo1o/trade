...
_문서 갱신: 2026-05-28 — 주문 멱등·`ledger_apply` 장부 저장·`balance_read` GUI/heartbeat 연동·`docs/idempotency/` 정합._
...
코드는 크게 **전략(`strategy/`)**, **실행·장부(`execution/`)**, **브로커 API(`api/`)**, **GUI·조회 보조(`services/`, `utils/`)** 로 나뉩니다. **중복 주문 방지(멱등)**·**장부 저장 재시도**·**잔고 TTL** 은 `execution/idempotency.py`, `execution/ledger_apply.py`, `execution/balance_read.py` 와 **`docs/idempotency/`** 설계 문서를 함께 보세요. 파일 단위 목차는 **`PROJECT_STRUCTURE.txt`**.
...
| **🔄 예수금 새로고침** | `BalanceUpdaterThread` → `build_account_snapshot_for_report(..., fresh_balances=True)` — 잔고는 **`execution/balance_read.py`** 경유(기존과 동일하게 API 재조회). 장중/쿨다운 정책·`last_kis_display_snapshot` 폴백은 그대로. |
...
1. **실시간 현황** 탭  
   - 보유 종목 **테이블(7열)**: 시장, 종목명, **수량(가운데 정렬)**, 매수가, **현재가**, **전략(V8 / 스윙)**, **수동 매도**(수량 입력 + 버튼). 최고가·매도선·보유시간은 이 탭에 두지 않습니다(아래 **장부**·텔레·15분 매매 로그).  
...
3. **장부 (현재 포지션)**  
   ...
   - **손절가 열:** `SWING_FIB`(또는 tier `SWING_FIB`)는 저장된 `sl_p` 대신 **현재가·일봉으로 `get_swing_exit_display_price` 를 재계산**해 표시합니다(러너 구간은 **5MA** 포함, 코인·국·미 동일).

4. **매매·전략 안내**  
   - 타임스탑, V8/스윙 매도선, **러너 5MA 트레일링**(1차 익절·1.5R 이후) — `strategy/rules.py` 상수와 동기화.
...
| **텔레그램 heartbeat** | **기동 직후 전송 없음.** 다음 **KST `:00` / `:30`** 슬롯을 예약한 뒤, **해당 슬롯의 15분 매매 사이클(`:00/:15/:30/:45`)이 끝난 다음** `heartbeat_report()` 를 보냅니다(UI 스레드 블로킹 방지용 백그라운드 스레드). 보유 한 줄에 **`전략:V8`** 또는 **`전략:스윙`** 이 포함됩니다(장부 `strategy_type`·`tier` 우선, 없으면 `trade_history` 최근 BUY `reason` 폴백). **GUI 하단 로그**에도 같은 생존신고·보유 블록이 heartbeat 시점에만 붙습니다(잔고 새로고침마다 반복하지 않음). |
...
├── execution/          # 동기화, TWAP, 멱등·장부저장·잔고TTL, 분할익절, 서킷
...
아래는 **한 번의 `run_trading_bot()`** 이 개념적으로 하는 일입니다. (실제 코드: `_prepare_cycle_state` → `_sync_positions_for_cycle` → `load_state` 후 **`reconcile_positions_for_cycle`** → 컨텍스트 → 시장별 루프.)

**멱등 vs 포지션 장부:** `order_idempotency` 는 **주문·체결 기록**, `positions` 는 **전략 상태** — 자동 1:1 연동은 하지 않습니다. 체결은 됐는데 `positions` 저장만 실패한 경우는 사이클 시작 정합 + 매도 직후 `ledger_apply` 검증 저장으로 보정합니다(`docs/idempotency/LEDGER_RECONCILE.md`).

```mermaid
flowchart TD
  A[사이클 시작 load_state] --> A2[멱등 prune + filled↔positions 정합]
  A2 --> B[실계좌와 장부 동기화 sync_all_positions]
...
**매도 쪽 주의:** 포지션마다 `strategy_type` 이 있습니다. **`SWING_FIB`** 는 `check_swing_exit`(HALF·피보/구름 FULL·**러너 5MA FULL**·RSI FULL) → (HOLD 시) 분할 익절 → 타임스탑 → 수익 구간 트레일링(`check_swing_profit_lock_trailing_exit`: **비러너**는 본절 락>3% 이탈, **러너**는 5MA 이탈). **피보·구름·5MA FULL은 `check_swing_exit` 전담**이며, 손실 구간 V8식 `hard_stop` 루프는 **스윙에 적용하지 않습니다.** 표시·`sl_p` 는 `get_swing_exit_display_price`(하드+본절 락, 러너 시 **+5MA**)로 갱신. **`TREND_V8`** 은 분할 익절·타임스탑·`get_final_exit_price`(샹들리에·20MA/ATR·본절 락≥4%)·`check_pro_exit` — **평단 대비 고정 % 절대 캡 없음**(§8).

**매수 쪽:** 시장별 스캔 대상은 **10일 RS**로 정렬한 뒤(`strategy/alpha_sizing.py`), 종목마다 **먼저 V8**(`calculate_pro_signals`, Hurst **&lt; 0.45** 차단)을 보고, 실패 시 **스윙 보조**(`check_swing_entry`)를 봅니다. 통과 후 비중은 **`min(1/N, alpha_target_vol/ATR%)`** 로 잡고, 해당 시장 **Portfolio Heat**(Σ 비중×ATR%)가 `portfolio_heat_max_pct`(기본 6%)를 넘으면 **그 시장 신규 매수 전체를 차단**합니다(V8·스윙 공통). 이후 `macro_mult`·예수금·최소주문을 적용합니다. **코인(업비트·바이낸스 공통)** 도 동일한 **일봉 직전 KST 창** 안에서만 매수 판단하며, 진입 순서도 국·미와 같습니다.

**Phase4 거시:** `_build_market_context` → `get_macro_guard_snapshot` 이 **시장별 글로벌 알파 차단**만 판정합니다. `market_buy_allowed[시장] == false` 이면 **V8·SWING_FIB 불문** 해당 시장 신규 매수 전면 중단(US PCR 스윙 예외 없음). `macro_mult`는 항상 **1.0**입니다.
...
매수·매도·RS·날씨(ADX) 판단에 쓰는 **일봉**은 `run_bot.py` 의 **`get_cached_ohlcv(ticker, broker=None)`** 로 모읍니다. 목표는 **200봉**이며, 확보한 뒤 메모리·`data/ohlcv_cache/` 디스크에 저장해 Yahoo 401 등으로 다음 사이클이 막히지 않게 합니다.

**조회 순서**

| 단계 | 내용 |
|------|------|
| 1 | 프로세스 메모리 캐시(`_ohlcv_cache`) — 이미 **≥200봉**이면 즉시 반환 |
| 2 | 디스크 `data/ohlcv_cache/{티커}.json` — **≥200봉**·3일 이내면 사용 |
| 3 | **국장(숫자 6자리 티커 + `broker` 전달):** KIS 국내 일봉 → **pykrx** |
| 4 | **미장(알파벳 티커, 주말 KIS 억제 창이 아닐 때):** KIS 해외 일봉 |
| 5 | **`config.json`의 `stooq_apikey`가 있을 때:** Stooq CSV (미장·일부 심볼 보강) |
| 6 | **yfinance** — 위에서 200봉 미만이면 마지막 폴백 (`utils/yfinance_guard.py`: 티커별 ~90초 백오프·stderr 억제, **전역 30분 차단 없음**) |
...
| `SWING_PROFIT_LOCK_ACTIVATE_PCT` | 3.0 | max_p 최고수익 **>3%** 일 때만 본절 락 |
| `V8_PROFIT_LOCK_ACTIVATE_PCT` | 4.0 | max_p 최고수익 **≥4%** 일 때만 본절 락 |
...
**기존 보유 종목:** 매도 루프마다 `get_swing_exit_display_price`·`get_swing_hard_stop_floor`(영업시간 `trading_hours_held` 반영)로 재계산하고 `sl_p`·`max_p` 를 갱신합니다. `entry_fib_level` 이 비어 있으면 `reconcile_swing_position` 이 60봉·평단으로 피보를 백필합니다. GUI 장부 탭도 스윙 종목은 **현재가·일봉으로 매도선을 재계산**해 표시합니다.
...
본절 락      = max_p 최고수익 ≥ +4% → 평단×1.005, 미만 → 0
최종 매도선  = max(샹들리에, 기술 매도선, 본절 락, 장부 sl_p 폴백)
```

1. **샹들리에 바닥:** `max_p`는 장부 최고가·현재가 중 큰 값.
2. **기술 매도선:** 매수 시그널과 동일한 20MA·ATR 구조.
3. **본절 락:** +10% 미만 구간에서도 타이트한 티어 락 없음 — 최고수익 **+4% 도달 후**에만 평단+0.5% 방어.
4. **수익 구간 청산:** `check_pro_exit` — 현재가 ≤ 최종 매도선이면 전량(본절 락이 샹들리에보다 높을 때 락 사유)
...
**매도 루프 우선순위 (V8):** 분할익절 → 타임스탑 → **손실 시** `hard_stop`(`get_final_exit_price` 갱신 `sl_p`) → **수익 시** `check_pro_exit`(샹들리에).
...
| **KR** | 원/달러 모멘텀 | `fetch_usd_krw_momentum()` (현재가÷5일 이평) | 모멘텀 **≥ 1.015** → KR 신규 매수 차단 |

**전략 예외 없음:** `market_buy_allowed[시장] == false` 이면 KR·US·COIN 모두 **V8·SWING_FIB** 신규 매수를 매수 창에서 즉시 중단합니다(`_macro_market_buy_allowed`). `macro_buy_allowed_for_strategy` 는 `market_buy_allowed` 와 동일하게만 동작합니다(호환 래퍼).

- 사이클 시작: `🛡️ [Phase4 거시]` + `🛡️ [Phase4 글로벌]` + 차단 시 `🚫 [Phase4 글로벌] {시장} 신규 매수 차단`.
- **실제 적용:** KR·US·COIN 공통 — Phase4 차단 시 **매수 창 전체 스킵**(BEAR 날씨의 V8-only 차단과 별개).
...
| `balance_read.py` | KR/US/COIN 잔고 12초 TTL — TWAP 검증·**GUI 스냅샷·`get_safe_balance`** |
...
- **잔고:** TWAP는 `refresh=False` → 주문 후 `refresh=True` 1회. **GUI 새로고침**은 `fresh_balances=True`(항상 재조회). **heartbeat** 는 TTL 공유(12초, API 절감). `BALANCE_READS.md` 참고.
