...
_문서 갱신: 2026-06-06 — 모듈화·장부 `.bak`·매매 로직(사이클 순서·SWING/V8·TWAP 역할) 명확화._
...
- **Phase4·알파 사이징:** `_build_market_context` 가 `macro_snap`(PCR·고래·환율 모멘텀·`market_buy_allowed`)을 넘깁니다. **KR RS**는 `_sort_buy_targets_by_rs` 로 사이클 시작 시; **US RS**는 `us_buy_cycle` 내부. `_position_ratio_with_vol_target` 로 변동성 타겟 비중.
...
- **`TREND_V8`** 만 **V8 매도 루프**의 **분할 익절(Scale-Out)** → 타임스탑 → 하드스탑 → 샹들리에 순을 탑니다.
  - **1차:** ``entry_atr×3.0`` 도달 시 50% → ``scale_out_done`` (ATR 없을 때만 +30% fallback)
...
- **Phase4 글로벌:** 사이클 시작 `🛡️ [Phase4 글로벌] PCR=… 고래롱숏=… 환율모멘텀=…`, 차단 요약 `🚫 [Phase4 글로벌] KR|US|COIN 신규 매수 차단`. **매수 창 안** Phase4+헷지: `🚨 [Phase 4 발동] …`, `🛡️ [헷지 유니버스 KR|US] 코드=한글명 … (수정: strategy/hedge_universe.py)`.
