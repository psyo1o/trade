...
4. **전략 계산 안내**  
   - 타임스탑(주식 휴장일 pause·코인 연속), V8/스윙 매도선·본절락 요약(운영 참고용, 코드 상수와 동기화 목적).
...
**매도 쪽 주의:** 포지션마다 `strategy_type` 이 있습니다. **`SWING_FIB`** 는 `check_swing_exit`(HALF·피보/구름 FULL·RSI FULL) → (HOLD 시) 분할 익절 → 타임스탑 → **본절 락 이탈만**(`check_swing_profit_lock_trailing_exit`: max_p 최고수익 **>3%** 후 평단×1.005). **피보·구름 FULL은 `check_swing_exit` 전담**이며, 손실 구간 V8식 `hard_stop` 루프는 **스윙에 적용하지 않습니다.** 표시·`sl_p` 는 `get_swing_exit_display_price`(피보·구름+본절 락)로 갱신. **`TREND_V8`** 은 분할 익절·타임스탑·`get_final_exit_price`(샹들리에·20MA/ATR·본절 락≥4%)·`check_pro_exit` — **평단 대비 고정 % 절대 캡 없음**(§8).
...
| 구분 | 방향 | 동작 | 장부·화면 |
|------|------|------|-----------|
| **매도선(스윙)** | 가격이 **아래로** 이 선 이하 | 시간가중 하드 바닥·수익 보존 락 → **잔량 전량** 매도 | `sl_p`, GUI 손절가 열, 생존신고 `매도선` |
| **1차 익절(1.5R)** | 수익이 **1.5R** 이상 | `HALF` → **약 50%만** 익절, 잔량 유지 | 로그 `1.5R 1차 익절` (매도선과 별개) |
...
|------|---------------------------|----------------|
| 손실·저수익 | `max(피보, 구름)` + **영업 24h 후 시간가중 상향** | FULL → **`check_swing_exit`** |
| max_p 최고수익 **>3%** | 평단×1.005 본절 락 | 수익 락 이탈 → **`check_swing_profit_lock_trailing_exit`** |
...
| 2 | **HALF** 1차 익절 | `(현재가 − 평단) ≥ 1.5 × entry_initial_risk_1r` | 50% 분할, `scale_out_done` 후 재발동 없음 |
...
시간가중  = 영업 24h 경과 후, 영업 24h마다 (평단 − 기술 바닥) gap의 10% 상향 (최대 평단 근처)
...
```
본절 락     = max_p 최고수익 > +3% → 평단×1.005, 이하 → 0
매도선(표시) = max( 하드 바닥, 본절 락 )   ← 락 미활성 시 피보·구름만
```
...
**Yahoo Finance (`yfinance`):** 일봉 **마지막 폴백**·RS 벤치마크·일부 환율 추정에 쓰이며 **공식 API가 아니라** 401·`Invalid Crumb`가 날 수 있습니다(증권 API 오류와 무관). **국장은 KIS·pykrx·디스크 캐시를 먼저** 쓰므로, Yahoo가 막혀도 **200봉·120MA 매수는 가능한 경우가 많습니다.** 완화: `pip install -U "yfinance>=0.2.48"`, **`krw_per_usdt` 직접 입력**, (미장) **`stooq_apikey`**, `utils/yfinance_guard.py`의 티커별 백오프·GUI/텔레 `is_yahoo_noise_line` 필터. **전역 yfinance 30분 차단은 사용하지 않습니다**(한 티커 실패가 다른 티커 매수를 막지 않도록).
