...
_문서 갱신: 2026-06-04 — 매매내역→장부 복구·KIS 강제 새로고침(총평 방어·장부 동기화)·GUI 로그 15px._
...
- **`TREND_V8`** 만 **V8 매도 루프**의 **분할 익절(Scale-Out)** → 타임스탑 → 하드스탑 → 샹들리에 순을 탑니다.
  - **1차:** ``entry_atr×3.0`` 도달 시 50% → ``scale_out_done``
...
- **1차:** `SCALE_OUT_ENTRY_ATR_MULT` (3.0) · **2차:** `SCALE_OUT_SECOND_ENTRY_ATR_MULT` (6.0), `second_scale_out_done`, 멱등 `scale_out_2`.
