...
- `execution/`
  - 실행/리스크/동기화 유틸.
  - `guard.py`: 장부 I/O, `cooldown` / `ticker_cooldowns`, MDD, 합산 서킷, `CORE_ASSETS` 단일 정의;
    매도 사유별 `ticker_cooldowns` 만료 시각 설정·조회 유틸.
...
- `CORE_ASSETS` 단일 소스: `execution/guard.py`
  - 주식 코어 + 코인 코어(`KRW-BTC`, `KRW-ETH`, `KRW-SOL`).
  - `run_bot.py` / `strategy/rules.py`는 `guard`에서 import해서 사용.

- `CORE_COIN_ASSETS`: `execution/guard.py`
  - `CORE_ASSETS` 중 `KRW-` 코인만 추출한 튜플.
  - 코인 스캔 폴백 대상 등에 재사용.
...
5. 매도(조건 충족 시) 후 신규 매수(시장·시간·슬롯 조건 충족 시).
...
### 8-5. 코어 자산 변경 시 어디를 수정?

- 단일 소스: `execution/guard.py`의 `CORE_ASSETS`.
- 다른 모듈은 import해서 사용하므로 여기만 변경하면 연동됨.
...
- 코어 자산 목록: `execution/guard.py`의 `CORE_ASSETS` 단일 관리.
- 코인 코어(BTC/ETH/SOL)는 `MAX_POSITIONS_COIN` 카운트에서 제외.
