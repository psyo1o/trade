...
## 2026-05-01 문서·운영 동기화 요약
- **운영 안정화(코드 반영됨, 모듈 분리와 별개):** 비장중 KIS 보유 조회 생략·장부 `held` 보강, 코인 업비트=바이낸스 동일 매수창·V8→스윙, 사이클 말 **매수 패스** 텔레그램. README §5·§6·§10·config 표, `PROJECT_STRUCTURE.txt` 동기화.
- **스윙 전략 정합성(2026-05, `strategy/rules.py`):** 피보는 현재가 **아래** 38.2/50/61.8%만·KR/US/COIN `reference_close` 실시간 양봉·윗꼬리 5% 진입 차단·볼밴 HALF(현재가≥상단+평단+2%)·RSI FULL(평단↑). 상세는 **README §8**.
- **로드맵 우선순위는 불변:** 여전히 시장별 사이클 분리(P0) → 전략 라우터(P0)가 첫 축. 위 운영 변경은 `run_bot.py`·`execution/sync_positions.py` 에 집중되어 있어 **향후 `_run_*_cycle` 분리 시 함께 들여올 테스트 후보**(매수 패스 조건·동기화 게이트)가 추가됨.
...
### P2. 리포트/알림 포맷터 분리
- 목표:
  - heartbeat·수동매도·오류·**매수 패스** 등 알림 문자열 조립을 엔진 로직에서 분리
- 제안 파일:
  - `services/report_formatter.py`
- 기대 효과:
  - 로그/메시지 변경 시 트레이딩 로직 영향 최소화
- 비고(2026-05-01):
  - 현재 매수 패스는 `run_trading_bot` 말미에서 `send_telegram` 문자열 리터럴 — 포맷터 도입 시 `_cycle_buy_*` 플래그를 넘겨 동일 문구를 생성하면 됨.
