...
class RedirectText(QObject):
    ...
    def __init__(self):
        ...
        self.log_file = None
        try:
            # main64의 로그 파일 경로를 가져와서 파일 열기
            if hasattr(main64, 'log_filename'):
                self.log_file = open(main64.log_filename, 'a', encoding='utf-8')
        except Exception as e:
            print(f"Log file open fail: {e}")
...
class BotDashboard(QMainWindow):
        ...
        for item in history:
            ...
            if ticker:
                ...
                if market in ("KR", "🇰🇷 국장") or ticker.isdigit():
                    ...
                    if not symbol_name:
                        try:
                            symbol_name = str(main64.get_kr_company_name(ticker) or "")
                        except Exception:
                            symbol_name = ""
                    ...
                    if not symbol_name:
                        try:
                            symbol_name = str(main64.get_us_company_name(ticker) or "")
                        except Exception:
                            symbol_name = ""
        ...
        for ticker, pos_info in positions.items():
            ...
            if ticker.isdigit():
                ...
                if not name or name == ticker:
                    try:
                        name = main64.get_kr_company_name(ticker)
                    except:
                        name = ticker
                ...
                if not name or name == ticker:
                    try:
                        name = main64.get_us_company_name(ticker)
                    except:
                        name = ticker
            ...
            try:
                if isinstance(buy_time, (int, float)):
                    from datetime import datetime
                    buy_time_str = datetime.fromtimestamp(buy_time).strftime("%m-%d %H:%M")
                else:
                    buy_time_str = str(buy_time)[:16]
            except:
                buy_time_str = ""
            self.ledger_table.setItem(row, 7, QTableWidgetItem(buy_time_str))
        ...
        try:
            ...
            try:
                ...
                for item in us_data:
                    ...
                    current_p = item.get('current_p', 0.0)
            ...
            self.refresh_ledger()  # 장부(포지션) 탭도 함께 갱신
    ...
    def add_table_row(self, market, name, qty, price, market_code, ticker_code, current_p=None):
        ...
        code_text = str(ticker_code or "").strip()
        if name_text and code_text and name_text != code_text:
            display_name = f"{name_text}({code_text})"
        else:
            display_name = code_text or name_text
        ...
        try:
            ...
            if market == "🇰🇷 국장":
                ...
                # [국장] 현재가 갱신 시 최고가(max_p) 업데이트 체크
                if current_price > 0:
                    self.update_max_price_if_higher(ticker_code, current_price)

            elif market == "🇺🇸 미장":
                # US: 1순위: KIS API에서 받은 현재가 사용
                if current_p and float(current_p) > 0:
    ...
    def update_max_price_if_higher(self, ticker, current_p):
        """
        GUI에서 확인된 현재가가 장부상 최고가보다 높으면 업데이트합니다.
        (유저 요청: 캐시에 넣었다가 확인해서 갱신, 로그 출력)
        """
        try:
            # 1. 상태 로드
            ...
            # [추가] 장 운영 시간 체크 (장이 열렸을 때만 최고가 갱신)
            market_type = "US"
            if ticker_key.startswith("KRW-"): 
                market_type = "COIN"
            elif ticker_key.isdigit(): 
                market_type = "KR"
            
            # 코인은 24시간이라 항상 True지만, 함수를 통해 일관성 유지
            if not main64.is_market_open(market_type):
                return
            ...
            if ticker_key in positions:
                ...
                # 3. 갱신 조건 확인 (1% 이상 차이날 때만 로그 찍거나 하는 게 좋겠지만, 일단 갱신 시 무조건)
                if current_p_float > old_max:
                    ...
                    # 갱신 불필요 (현재가가 최고가보다 낮음 - 로그 출력 요청 반영)
                    msg = f"🔍 [GUI 점검] {ticker_key} 최고가 유지 (현재: ${current_p_float:.2f} <= 최고: ${old_max:.2f})"
                    print(msg)

        except Exception as e:
            # 6. 로그 출력 (실패)
            print(f"⚠️ 최고가 갱신 중 오류 ({ticker}): {e}")
