...
def load_state(path: Path):
    ...
    * 깨진 원본은 ``*_corrupt_타임스탬프.bak`` 으로 남긴다.
    ...
    if not isinstance(data, dict):
        return _finalize_loaded_dict(dict(empty))
    fin = _finalize_loaded_dict(data)
    try:
        from execution.state_schema import log_load_summary

        log_load_summary(fin, path_label=str(path.name))
    except Exception:
        pass
    return fin


def _backup_state_file_before_save(path: Path) -> None:
    """positions 가 있을 때 저장 직전 ``.bak`` 에 현재 파일 복사 (장부 유실 완화)."""
    from execution.state_schema import count_positions
...
def save_state(path: Path, state):
    ...
    저장 직전: 기존 파일에 positions 가 있으면 ``.bak`` 복사. positions 급감 시 경고 로그.
    """
    path = Path(path)
    from execution.state_schema import count_positions, warn_positions_drop

    prev_n = 0
    try:
        if path.is_file():
            prev_raw = path.read_text(encoding="utf-8")
            if prev_raw.strip():
                prev_n = count_positions(json.loads(prev_raw))
    except Exception:
        prev_n = 0
    try:
        warn_positions_drop(
            prev_count=prev_n,
            new_count=count_positions(state if isinstance(state, dict) else {}),
            context="save_state",
        )
    except Exception:
        pass
    _backup_state_file_before_save(path)
